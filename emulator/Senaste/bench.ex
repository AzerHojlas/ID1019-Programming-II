defmodule Bench do
  ...
  def rand(r, k) do # determines if instructins should be read or write
    i = (:rand.uniform(k) - 1) # random number from 0 to (k - 1)
  ...
  def dummy(reg, op) do # will execute in constant time no matter the arguments
    case op do
      {:read, _key} ->
	reg
      {:write, 0} ->
